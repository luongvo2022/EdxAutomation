...
import selenium4.com.enums.FailureHandling;
import selenium4.com.reports.ExtentReportManager;
import selenium4.com.reports.ExtentTestManager;
...
/**
 * WebElementsHelpers là class chung làm thư viện xử lý sẵn với nhiều hàm custom
 * từ Selenium và Java. Trả về là một Class chứa các hàm Static. Gọi lại dùng
 * bằng cách lấy tên class chấm tên hàm (WebElementsHelpers.method)
 */
public class WebElementsHelpers {
	...
	public static boolean verifyElementTextEquals(By by, String text, FailureHandling flowControl) {
		...
		if (flowControl.equals(FailureHandling.CONTINUE_ON_FAILURE)) {
			softAssert.assertEquals(getTextElement(by).trim(), text.trim(),
					"The actual text is '" + getTextElement(by).trim() + "' not equals '" + text.trim() + "'");
            if (result == false) {
	...
	public static WebElement highLightElement(By by) {
		...
		if (DriverManager.getDriver() instanceof JavascriptExecutor) {
			((JavascriptExecutor) DriverManager.getDriver()).executeScript("arguments[0].style.border='3px solid red'",
					waitForElementVisible(by));
			sleep(1);
	...
	@Step("Set text on textbox by Javascript {document.getElementById}")
	public static void setTextWithJs(String elementId, String text) {
	...
	@Step("Set text on textbox with Actions class")
	public static void setTextWithActions(By by, String value) {
		waitForElementVisible(by);
		// Scroll to element với Javascript Executor`
		JavascriptExecutor js = (JavascriptExecutor) DriverManager.getDriver();
		js.executeScript("arguments[0].scrollIntoView(true);", getWebElement(by));
	...
	@Step("Get attribute {1} of element {0}")
	public static String getAttributeElement(WebElement element, String attributeName) {
		smartWait();
	...
	public static ArrayList getValueTableByColumn(int column) {
		...
		sleep(1);
		LogUtils.info("Number of results for column (" + column + "): " + totalRows.size()); // Không thích ghi log thì
																								// xóa nhen
	...
	public static boolean waitForNumberOfWindowsToBe(int NbrOfWindows) {
		smartWait();
		...
		try {
			...
			Boolean flag = false;
			int counter = 0;
			while (!flag && counter <= WAIT_EXPLICIT) {
				try {
			...
			LogUtils.error("Timeout for number of windows to be " + NbrOfWindows);
			Assert.fail("Timeout for number of windows to be " + NbrOfWindows);
		}
		return false;
	}

	public static List<WebElement> waitForNumberOfElementsToBe(By by, Integer num) {
		smartWait();

		try {
			WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(WAIT_EXPLICIT),
					Duration.ofMillis(500));
			return wait.until(ExpectedConditions.numberOfElementsToBe(by, num));
		} catch (Throwable error) {
			LogUtils.error("Element not exist. " + by.toString());
			Assert.fail("Element not exist. " + by.toString());
		}
	...
	public static List<WebElement> waitForNumberOfElementsToBeMoreThan(By by, Integer num) {
		smartWait();

		try {
			WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(WAIT_EXPLICIT),
					Duration.ofMillis(500));
			return wait.until(ExpectedConditions.numberOfElementsToBeMoreThan(by, num));
		} catch (Throwable error) {
			LogUtils.error("Element not exist. " + by.toString());
			Assert.fail("Element not exist. " + by.toString());
		}
	...
	public static boolean verifyElementAttributeValue(By by, String attribute, String value) {
		...
		try {
			...
			LogUtils.error("Object: " + by.toString() + ". Not found value: " + value + " with attribute type: "
